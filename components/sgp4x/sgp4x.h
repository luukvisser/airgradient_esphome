// Local fork of esphome/components/sgp4x with two added outputs:
//   - voc_raw: SRAW_VOC ticks from the SGP41 (last value read each second)
//   - nox_raw: SRAW_NOX ticks from the SGP41 (last value read each second)
//
// These are needed so the cloud uploader can populate `tvocRaw` and `noxRaw`
// to match the official AirGradient Arduino library payload.
//
// Note on terminology: "raw" here means *before the Sensirion gas index
// algorithm* (no learning curves, no 0-500 mapping) — NOT before sensor
// compensation. Both this component and the AirGradient Arduino library
// pass RH and Temp ticks into the SGP41_CMD_MEASURE_RAW (0x2619) command,
// so the chip applies humidity/temperature compensation internally before
// returning SRAW_VOC / SRAW_NOX. The values published here are therefore
// equivalent to what the Arduino library reports as tvocRaw / noxRaw.
//
// Diff vs upstream: only the two `set_voc_raw_sensor` / `set_nox_raw_sensor`
// setters, the matching member pointers, and the publish lines in update().
// Everything else is identical to upstream so future merges are mechanical.
...
class SGP4xComponent : public PollingComponent, public sensor::Sensor, public sensirion_common::SensirionI2CDevice {
 ...
 public:
  ...
  void set_voc_raw_sensor(sensor::Sensor *voc_raw_sensor) { voc_raw_sensor_ = voc_raw_sensor; }
  void set_nox_raw_sensor(sensor::Sensor *nox_raw_sensor) { nox_raw_sensor_ = nox_raw_sensor; }
 ...
 protected:
  ...
  sensor::Sensor *voc_raw_sensor_{nullptr};
  ...
  sensor::Sensor *nox_raw_sensor_{nullptr};
