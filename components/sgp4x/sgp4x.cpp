...
void SGP4xComponent::update() {
  ...
  // Local addition: publish raw SRAW signals when bound. The conditioning
  // phase reuses the SGP41_CMD_NOX_CONDITIONING command which only returns
  // the VOC raw value, so nox_sraw_ stays at 0 during the first ~10 s.
  if (this->voc_raw_sensor_ != nullptr) {
    this->voc_raw_sensor_->publish_state(this->voc_sraw_);
  }
  if (this->nox_raw_sensor_ != nullptr) {
    this->nox_raw_sensor_->publish_state(this->nox_sraw_);
  }
}
...
void SGP4xComponent::dump_config() {
  ...
  LOG_SENSOR("  ", "VOC Raw", this->voc_raw_sensor_);
  LOG_SENSOR("  ", "NOx Raw", this->nox_raw_sensor_);
}
