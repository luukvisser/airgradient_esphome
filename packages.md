...
Adds a switch that reboots the device into safe mode, which disables most components to
allow OTA flashing when a misbehaving component or low-memory condition is blocking
normal OTA.
...
By default the sensor wakes every 2 minutes to take a reading, extending its rated
lifespan. The update interval is configurable:
...
Same extended duty-cycle and calibration substitutions as
`sensor_pms5003_extended_life.yaml`.
...
- **CO2 offset** — shift the reading by a known number of ppm (positive or negative):
  ...
  substitutions:
    co2_offset: "0"
  ```
- **ABC interval** — Automatic Baseline Correction period (1, 8, 30, or 90 days; default
  8).
...
Calculates US EPA AQI and NowCast AQI entirely on-device, using a 24-hour rolling window
of PM2.5 readings and the 2024 EPA breakpoints. Also emits an AQI category string (Good,
Moderate, …, Hazardous). Contributed by [@Ex-Nerd](https://github.com/Ex-Nerd).
...
SH1106 128×64 OLED display with ten independently switchable pages. Each page is toggled
by a Home Assistant switch (or the web UI):
...
Multi-mode indicator. Depends on `led.yaml`. Adds a **LED Mode** selector with six
options:
...
Color thresholds are configurable via substitutions (defaults follow
ASHRAE/WHO/Sensirion guidelines — see `README.md`):
...
[AirGradient Dashboard](https://app.airgradient.com/dashboard) every 2.5 minutes via
HTTPS. A boot POST is sent on startup.

Includes a **Send to AirGradient** toggle switch so data upload can be disabled without
reflashing.
